...
}).refine(({ year, month, day }) => day <= ICAL.Time.daysInMonth(month, year), {
    message: 'day cannot be greater than there are days in the given month'
});
const signet = fs.readFileSync(path.resolve('public/logo.txt')).toString();
const favicon = fs.readFileSync(path.resolve('public/favicon.ico'));
