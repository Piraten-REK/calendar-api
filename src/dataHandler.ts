...
class DataHandler {
  ...
  async #update (): Promise<void> {
    await getNewData()
      .then(({ singleEvents, recurringEvents, timezone }) => {
        this.#data = new Map<number, Map<number, Month>>()

        for (const event of singleEvents) {
          for (const [year, month] of this.#whichMonths(event)) {
            if (!this.#data.has(year)) {
              this.#data.set(year, new Map<number, Month>())
            }
            if (!(this.#data.get(year) as Map<number, Month>).has(month)) {
              (this.#data.get(year) as Map<number, Month>).set(month, new Month(year, month, [], timezone))
            }
  ...
  #whichMonths (event: Event): Array<[number, number]> {
    ...
    return arr.filter((val, idx, self) =>
      self.findIndex(val2 => val[0] === val2[0] && val[1] === val2[1]) === idx
    )
  }
   ...
   */
  getMonth (year: number, month: number): ReturnType<Month['toJSONObject']> {
    if (!this.#data.has(year)) {
      this.#data.set(year, new Map<number, Month>())
    }
    if (!(this.#data.get(year) as Map<number, Month>).has(month)) {
      (this.#data.get(year) as Map<number, Month>).set(month, new Month(year, month, [], this.#timezone))
    }

    return ((this.#data.get(year) as Map<number, Month>).get(month) as Month).toJSONObject(this.#recurringEvents)
  }
   ...
   */
  getDay (year: number, month: number, day: number): ReturnType<Month['dayToJSONObject']> {
    if (!this.#data.has(year)) {
      this.#data.set(year, new Map<number, Month>())
    }
    if (!(this.#data.get(year) as Map<number, Month>).has(month)) {
    ...
    return ((this.#data.get(year) as Map<number, Month>).get(month) as Month).dayToJSONObject(day, this.#recurringEvents)
  }
}
