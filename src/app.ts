...
import { errorHandler, formatOrdinals, sendProblem, sendZodError } from './helpers.js'
import { NEXT_DEFAULT } from './config.js'
...
app.use((req, res, next) => {
  ...
  res.header('Cache-Control', 'max-age=300, s-maxage=600, must-revalidate, public')
  next()
})
...
app.get('/next', async (req, res, next) => {
  try {
    const nQuery = req.query.n
    const nInt = parseInt((Array.isArray(nQuery) ? nQuery.at(-1) : nQuery) as string)
    const n = isNaN(nInt) ? NEXT_DEFAULT : nInt
...
app.get('/:param0/:param1', (req, res, next) => {
  try {
...
app.get('/:param0/:param1/:param2', (req, res, next) => {
  try {
...
app.get('/:param0/:param1/:param2/:param3', (req, res, next) => {
  try {
